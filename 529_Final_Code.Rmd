...
#test example of estimatr regression
...
#write function to calculate regressions
regression_function <- function(dependent_variable, common_support, policy_variable, log = TRUE){
...
#function to create nice regression outputs
regression_table_function <- function(model_output, dependent_variable, policy_variable) {
  tab_model(model_output,
          show.p = TRUE, p.style = "asterisk", show.obs = FALSE,
          terms = dependent_variable,
          title = policy_variable)
}
```

```{r create txt file of estimatr regression results}
#list of variables to go through
list_of_variables_and_policies <- data.frame("id" = 1:24,
                                             "dependent_variable" = rep(c("marijuana_possession_arrest_rate",
                                                                      "marijuana_possession_and_sale_arrest_rate",
                                                                      "black_marijuana_possession_arrest_rate",
                                                                      "black_marijuana_possession_and_sale_arrest_rate",
                                                                      "marijuana_possession_arrest_rate_disparity",
                                                                      "marijuana_possession_and_sale_arrest_rate_disparity",
                                                                      "non_drug_misdemeanor_arrest_rate",
                                                                      "black_non_drug_misdemeanor_arrest_rate",
                                                                      "non_drug_misdemeanor_arrest_rate_disparity",
                                                                      "total_arrest_rate",
                                                                      "black_arrest_rate",
                                                                      "total_arrest_rate_disparity"
                                                                      ),2),
                                             "policy_variable" = c(rep("decriminilization_status_fraction_of_year_legalized_by_city",
                                                                     12),
                                                                   rep("legalization_status_fraction_of_year_legalized", 12)),
                                             stringsAsFactors = FALSE)


sink("Regression_results/common_support.txt")
for(i in 1:nrow(list_of_variables_and_policies)){
  regression_function(list_of_variables_and_policies$dependent_variable[i],
                      common_support_cities,
                      list_of_variables_and_policies$policy_variable[i],
                      log = FALSE) %>% 
    print()
  }
sink()

sink("Regression_results/all_cities.txt")
for(i in 1:nrow(list_of_variables_and_policies)){
   regression_function(list_of_variables_and_policies$dependent_variable[i],
                      all_cities,
                      list_of_variables_and_policies$policy_variable[i],
...
sink("Regression_results/common_support_log.txt")
for(i in 1:nrow(list_of_variables_and_policies)){
  regression_function(list_of_variables_and_policies$dependent_variable[i],
                      common_support_cities,
                      list_of_variables_and_policies$policy_variable[i],
...
sink("Regression_results/all_cities_log.txt")
for(i in 1:nrow(list_of_variables_and_policies)){
   regression_function(list_of_variables_and_policies$dependent_variable[i],
                      all_cities,
                      list_of_variables_and_policies$policy_variable[i],
...
UCR_arrest_totals_and_rates_with_pop %>% 
  ...
  ggplot(aes(x = Year, y = (marijuana_possession_and_sale_arrest_rate))) +
  ...
  scale_color_discrete(name = "Decriminalization status:", labels = c("Did not decriminalize in that year", "Decriminalized by end of that year")) +
  ...
  #labs(title = "Marijuana possession and sale arrest rates, 2009 - 2016",
       #subtitle = "Only includes cities that decriminalized marijuana before 2016, in the common support region") +
  theme_minimal() +
...
UCR_arrest_totals_and_rates_with_pop %>% 
  ...
  ggplot(aes(x = Year, y = (marijuana_possession_and_sale_arrest_rate))) +
  geom_line(aes(group = City,
                color = factor(legalization_status_one_equals_any_legalization_in_year)),
            size = .8) +
  facet_wrap(~State) +
  #geom_text_repel(aes(label = ifelse(Year == max(Year) & State != "California", as.character(City), NA_character_)), size = 3) +
  scale_color_discrete(name = "Decriminalization status:", labels = c("Did not decriminalize in that year", "Decriminalized by end of that year")) +
...
UCR_arrest_totals_and_rates_with_pop <- UCR_arrest_totals_and_rates_with_pop %>% 
  mutate(white_total_arrest_rate = total_White_arrests / total_arrests_race_data)

UCR_arrest_totals_and_rates_with_pop %>% 
  filter(ACS_city_name %in% common_support_cities) %>% 
  #group_by(Year) %>% 
  summarize(mean_arrest_rate = mean(total_arrest_rate),
            mean_white_arrest_rate = mean(white_total_arrest_rate),
            mean_black_arrest_rate = mean(black_arrest_rate),
            total_arrest_rate_disparity = mean(total_arrest_rate_disparity),
            marijuana_possession_arrest_rate = mean(marijuana_possession_arrest_rate),
            white_marijuana_possession_arrest_rate = mean(white_marijuana_possession_arrest_rate),
            black_marijuana_possession_arrest_rate = mean(black_marijuana_possession_arrest_rate),
            marijuana_possession_arrest_rate_disparity = mean(marijuana_possession_arrest_rate_disparity),
            non_drug_misdemeanor_arrest_rate = mean(non_drug_misdemeanor_arrest_rate),
            white_non_drug_misdemeanor_arrest_rate = mean(white_non_drug_misdemeanor_arrest_rate),
            black_non_drug_misdemeanor_arrest_rate = mean(black_non_drug_misdemeanor_arrest_rate),
            non_drug_misdemeanor_arrest_rate_disparity = mean(non_drug_misdemeanor_arrest_rate_disparity)) %>% 
  view()
...
UCR_arrest_totals_and_rates_with_pop %>% 
  filter(ACS_city_name %in% common_support_cities) %>% 
  #group_by(Year) %>% 
  summarize(total_violent_arrest_rate = mean(total_violent_arrest_rate),
            pct_black = mean(pct_black),
            pct_in_poverty = mean(pct_in_poverty),
            total_popE = mean(total_popE)) %>% 
  view()
...
UCR_arrest_totals_and_rates_with_pop %>% 
  filter(ACS_city_name %in% common_support_cities) %>% 
  group_by(State) %>% 
  summarize(pct_black = mean(pct_black),
            total_arrest_rate_disparity = mean(total_arrest_rate_disparity),
            marijuana_possession_arrest_rate_disparity = mean(marijuana_possession_arrest_rate_disparity),
            non_drug_misdemeanor_arrest_rate_disparity = mean(non_drug_misdemeanor_arrest_rate_disparity)) %>%
  view()
