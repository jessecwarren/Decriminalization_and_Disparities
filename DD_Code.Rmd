...
#replace all 99999, 99998 values, which code as missing, with NAs
...
UCR_data_list$UCR_2015 <- UCR_2015 %>%
  select(STATE, STNAME, OFFENSE, 34:89) %>% 
  group_by(STATE, STNAME, OFFENSE) %>%
...
vlist11 <- load_variables(2011, "acs5", cache = TRUE)
...
UCR_arrest_totals_and_rates_with_pop %>% 
  ...
  lm(marijuana_arrest_rate ~ legalization_status_fraction_of_year_legalized + factor(State) + factor(Year),
     data = .) %>% 
  summary()
...
UCR_arrest_totals_and_rates_with_pop %>% 
  filter(State %in% common_support_states) %>% 
  lm(black_marijuana_arrest_rate ~ legalization_status_fraction_of_year_legalized + factor(State) + factor(Year),
     data = .) %>% 
  summary()
