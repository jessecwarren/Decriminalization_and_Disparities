...
UCR_arrest_totals_and_rates_with_pop <- UCR_arrest_totals_and_rates %>% 
  ungroup() %>% 
  dplyr::select(-STNAME) %>% 
  dplyr::select(year, State, City, ACS_city_name, AGENCY, GROUP, GEOID, UCR_pop = POP, total_popE, white_popE, white_popM, black_popE, black_popM, UCR_to_ACS_pop_ratio, 5:19)
