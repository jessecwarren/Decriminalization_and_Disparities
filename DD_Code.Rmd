...
#To Do
  ...
  #fix issue with UCR crosswalk having duplicate counties
```
...
UCR_data_list$UCR_2015 <- UCR_2015 %>%
  dplyr::select(STATE, STNAME, COUNTY, OFFENSE, AGENCY, POP, 34:89) %>% 
  group_by(STATE, STNAME, COUNTY, OFFENSE, AGENCY, POP) %>%
...
ACS_state_pops <- map_dfr(
  pop_ACS_years, 
  ~ get_acs(geography = "state", variables = c(state_total_pop = "B03002_001", state_white_pop = "B03002_003", state_black_pop = "B03002_004"),
            year = .x, output = "wide"),
  .id = "year") %>% 
  dplyr::select(-state_total_popM) %>% 
  arrange(year, GEOID)

#Load census county population data found here https://www.census.gov/newsroom/press-kits/2018/estimates-characteristics.html
ACS_county_pops <- read_csv("Data/County_Population_Data/cc-est2017-alldata.csv") %>% 
  filter(AGEGRP == 0)

ACS_county_pops <- ACS_county_pops %>% 
  dplyr::select(STATE, COUNTY, STNAME, CTYNAME, YEAR,
                COUNTY_TOT_POP = TOT_POP,
                COUNTY_TOT_MALE = TOT_MALE,
                COUNTY_TOT_FEMALE = TOT_FEMALE,
                COUNTY_WA_MALE = WA_MALE,
                COUNTY_WA_FEMALE = WA_FEMALE,
                COUNTY_BA_MALE = BA_MALE,
                COUNTY_BA_FEMALE = BA_FEMALE) %>% 
  filter(YEAR == 3 | YEAR == 4 |YEAR == 5 | YEAR == 6 | YEAR == 7 | YEAR == 8 | YEAR == 9) %>% 
  mutate(STATE = as.numeric(STATE),
         COUNTY = as.numeric(COUNTY))

#recode year variable based on codebook
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 3] <- 2010
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 4] <- 2011
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 5] <- 2012
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 6] <- 2013
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 7] <- 2014
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 8] <- 2015
ACS_county_pops$YEAR[ACS_county_pops$YEAR == 9] <- 2016

ACS_county_pops$YEAR <- as.character(ACS_county_pops$YEAR)
...
#bind into one df
UCR_arrest_totals_and_rates_raw <- bind_rows(UCR_arrest_totals_and_rates, .id = "column_label") %>% arrange(column_label)

#save to csv to view later
write_csv(UCR_arrest_totals_and_rates_raw, "Calculated_UCR_Data.csv")

#create copy of df to manipulate while still preserving original
UCR_arrest_totals_and_rates <- UCR_arrest_totals_and_rates_raw
```
...
for(i in 1:length(UCR_data_list)){
  ...
  UCR_county_population_data[[i]] <- UCR_data_list[[i]] %>%
    ungroup() %>% 
...
#Load UCR County Codes to FIPS County and State Codes
UCR_County_Codes <- read.dta("Data/UCR_State_County_Codes/UCR_1990_County_Codes/DS0001/02565-0001-Data.dta") %>% 
  dplyr::select(ST_UCR, CTY_UCR, ST_FIPS, CTY_FIPS, CTYNAME)

#merge with UCR data
UCR_arrest_totals_and_rates <- UCR_arrest_totals_and_rates %>% 
  left_join(UCR_County_Codes, by = c("STATE" = "ST_UCR", "COUNTY" = "CTY_UCR"))
...
UCR_arrest_totals_and_rates_with_pop <- left_join(UCR_arrest_totals_and_rates_with_pop, ACS_county_pops, by = c("ST_FIPS" = "STATE", "CTY_FIPS" = "COUNTY", "year" = "YEAR"))
...
```{r remove states that don't report to UCR or have low percentage of agencies reporting in each county}
...
UCR_arrest_totals_and_rates_with_pop_filtered <- UCR_arrest_totals_and_rates_with_pop %>% 
  filter(!NAME %in% states_that_dont_report_to_UCR_or_underreport)

#calculate coverage ratio variable to find counties with low numbers of agencies reporting
UCR_arrest_totals_and_rates_with_pop_filtered <- UCR_arrest_totals_and_rates_with_pop %>% 
  mutate(county_coverage_ratio = UCR_Population / COUNTY_TOT_POP)

#remove counties with coverage ratio below 99%
counties_with_low_coverage_ratios_in_2011 <- UCR_arrest_totals_and_rates_with_pop_filtered %>% 
  filter(year == 2011) %>% 
  filter(county_coverage_ratio < .99)  %>% 
  pull(County)


UCR_arrest_totals_and_rates_with_pop_filtered <- UCR_arrest_totals_and_rates_with_pop_filtered %>% 
  filter(!County %in% counties_with_low_coverage_ratios_in_2011)
...
states_that_legalized_by_end_of_2016_vector <- UCR_arrest_totals_and_rates_with_pop_filtered %>% 
  filter(year == 2016) %>% 
  filter(legalization_status_one_equals_any_legalization_in_year == 1) %>% 
  dplyr::select(NAME) %>% 
  unique() %>% 
  pull(NAME)
states_that_legalized_by_end_of_2016_vector

#create vector of counties that legalized by end of 2016
counties_that_legalized_by_end_of_2016_vector <- UCR_arrest_totals_and_rates_with_pop_filtered %>% 
  filter(year == 2016) %>% 
  filter(legalization_status_one_equals_any_legalization_in_year == 1) %>% 
  dplyr::select(County) %>% 
  unique() %>% 
  pull(County)
counties_that_legalized_by_end_of_2016_vector
```
...
UCR_arrest_totals_and_rates_with_pop_filtered <- UCR_arrest_totals_and_rates_with_pop_filtered %>% 
  mutate(total_arrest_rate = total_arrests_race_data / COUNTY_TOT_POP) %>% 
  mutate(marijuana_arrest_rate = total_marijuana_arrests_race_data / COUNTY_TOT_POP) %>% 
  mutate(white_arrest_rate = total_White_arrests / sum(COUNTY_WA_MALE + COUNTY_WA_FEMALE)) %>% 
  mutate(black_arrest_rate = total_African_American_arrests / sum(COUNTY_BA_MALE + COUNTY_BA_FEMALE)) %>% 
  mutate(white_marijuana_arrest_rate = total_White_marijuana_arrests / sum(COUNTY_WA_MALE + COUNTY_WA_FEMALE)) %>% 
  mutate(black_marijuana_arrest_rate = total_African_American_marijuana_arrests / sum(COUNTY_BA_MALE + COUNTY_BA_FEMALE)) %>% 
  mutate(non_drug_misdemeanor_arrest_rate = total_non_violent_non_drug_misdemeanor_arrests_race_data / COUNTY_TOT_POP) %>% 
  mutate(white_non_drug_misdemeanor_arrest_rate = total_White_non_drug_misdemeanor_arrests / sum(COUNTY_WA_MALE + COUNTY_WA_FEMALE)) %>% 
  mutate(black_non_drug_misdemeanor_arrest_rate = total_African_American_non_drug_misdemeanor_arrests / sum(COUNTY_BA_MALE + COUNTY_BA_FEMALE))
...
UCR_arrest_totals_and_rates_with_pop_filtered %>% 
...
UCR_arrest_totals_and_rates_with_pop_filtered %>% 
  #filter(County %in% counties_that_legalized_by_end_of_2016_vector) %>% 
  ggplot(aes(x = Year, y = marijuana_arrest_rate)) +
...
```{r examine county pops delete later}
UCR_data_list$UCR_2011 %>% 
  filter(STNAME == "WASH", AGENCY == "KING" | AGENCY == "SEATTLE") %>% 
  view()

UCR_data_list$UCR_2014 %>% 
  filter(STNAME == "WASH", COUNTY == 17) %>% 
  dplyr::select(COUNTY, AGENCY, POP) %>% 
  unique() %>% 
  group_by(COUNTY) %>% 
  summarise(pop = sum(POP))

ACS_county_pops %>% 
  filter(STNAME == "Washington", CTYNAME == "King County")

head(UCR_data_list$UCR_2011)
